...
func (h *eventHandler) OnDDLAlter(mb [][]byte, db []byte) (rdb, table []byte, prefix, suffix [][]byte, err error) {
	...
	case "rename":
		...
		// invalid new table
		err2 := h.OnTableChanged(string(tdb), string(ttb))
		if err2 != nil && err2.Error() != canal.ErrExcludedTable.Error() {
			return nil, nil, nil, nil, errors.Trace(err1)
		}

		log.Debugf("rule db=%v , tb = %v", string(tdb), string(ttb))
		// check rule
		rule, ok := h.r.rules[ruleKey(string(tdb), string(ttb))]
		if !ok{
			log.Errorf("%s.%s not int rules", db, table)
			return nil, nil, nil, nil, errors.Errorf(fmt.Sprintf("%s.%s not int rules", db, table))
		}
...
func (h *eventHandler) OnDDL(nextPos *mysql.Position, db string, table string, prefix [][]byte, suffix [][]byte, isDrop bool) error {

	// check rule
	rule, ok := h.r.rules[ruleKey(db, table)]
	if !ok{
		return errors.Errorf("%s.%s not int rules", db, table)
	}
	...
	log.Infof("OnDDL sql = %v", buf.String())
	log.Infof("OnDDL pos = %s", nextPos)
